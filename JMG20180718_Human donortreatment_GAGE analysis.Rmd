---
title: "GAGE analysis_human_donor treatment"
output:
...
##Purpose: 
To assess if the differentially expressed human (also includes HBV) genes (when donor and treatment are set as factors in the design) are enriched for members of specific pathways. 
This analysis includes the July 2018 samples. 
...
write.xlsx(gage_mixed_df, file.path("GAGE analysis", paste(Sys.Date(), 
    "human_GAGE analysis_donortreatment_unfiltered.xlsx")))

##Filtering GAGE analysis based on q.val 
gage_mixed_df_filtered <- 
  dplyr::filter(gage_mixed_df, `HumanHBVgenes-HBV_vs_mock_d8.q.val` <= 0.07 |   
  `HumanHBVgenes-HBV_vs_mock_d28.q.val` <= 0.07 |
  `HumanHBVgenes-coinf_vs_mock_d8.q.val` <= 0.07 |
  `HumanHBVgenes-coinf_vs_mock_d28.q.val` <= 0.07 |
  `HumanHBVgenes-coinf_vs_HBV_d8.q.val` <= 0.07 |
  `HumanHBVgenes-coinf_vs_HBV_d8.q.val` <= 0.07)

##Put the columns in order of HBV v mock d8, d28; coinf v mock d8, d28; 
##coinf v HBV d8, d28.
colnames(gage_mixed_df_filtered)
gage_mixed_df_filtered <- gage_mixed_df_filtered[,c(1, 3, 2, 7, 6, 5, 4)] 

##Make data frame into matrix for heatmap.
gage_mixed_matrix <- as.matrix(gage_mixed_df_filtered[, c(2:7)])
...
colnames(gage_mixed_matrix) <-  gsub(pattern = "HumanHBVgenes-", replacement = "", 
                                     colnames(gage_mixed_matrix))
colnames(gage_mixed_matrix)
...
ggsave(filename = file.path("GAGE analysis", paste(Sys.Date(),
    "human_donortreatmentGAGE_plot.png")), matrix_plot, dpi = 350, height = 7, width = 10)
}
...
"human_GAGE analysis_donortreatment_filtered.xlsx")))
